...
def classify_image(image):
    ...
    prediction = {id2label[i]: round(probs[i]*100, 1) for i in range(len(probs))}
    return prediction
...
# --- Tab 2: Mini-Game ---
with tab2:
    st.header("Guess which image is AI-generated!")
        ...
        if len(ai_images) == 0 or len(real_images) == 0:
            ...
            if len(st.session_state.ai_deck) == 0 or len(st.session_state.real_deck) == 0:
                ...
                tips = [
                    ...
                    "Check for distorted facial proportions.",
                    "Eyes, ears, and teeth may appear distorted in AI images.",
                    "Shadows and lighting might look unnatural."
                ]
