...
# --- Helper: extract simple image features ---
def extract_features(img: Image.Image):
    img = img.resize((64,64)).convert("RGB")
    arr = np.array(img)
    return arr.flatten()

# --- Prepare training classifier ---
ai_folder = "ai_faces"
...
with tab1:
    ...
    if uploaded_file:
        ...
        try:
            ...
            prob = clf.predict_proba(features)[0][1]  # probability of AI
            verdict = "Likely AI" if prob > 0.5 else "Likely Real"
            st.success(f"{verdict} ({int(prob*100)}% confidence)")
            st.info("Tip: Look for weird blurs, distorted facial expressions, or misaligned features in AI-generated images.")
        except Exception as e:
            st.error(f"Could not analyze image. Error: {e}")
        ...
        if len(ai_images)==0 or len(real_images)==0:
            ...
            if len(st.session_state.ai_deck)==0 or len(st.session_state.real_deck)==0:
                ...
                if "left_img" not in st.session_state or not st.session_state.round_active:
                    if st.session_state.ai_deck and st.session_state.real_deck:
